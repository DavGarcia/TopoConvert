...
def register(cli):
    ...
    @click.option('--no-contours', is_flag=True,
                  help='Disable elevation contours overlay (default: show contours)')
    @click.option('--contour-interval', type=float, default=5.0,
                  help='Contour interval in feet (default: 5.0)')
    @click.option('--target-slope', type=float, default=None,
    ...
    def slope_heatmap(input_file, output_file, elevation_units, grid_resolution,
                     ...
                     no_contours, contour_interval, target_slope):
