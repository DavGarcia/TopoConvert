    ...
    def combined_dxf(csv_files, output, target_epsg, wgs84):
        ...
        try:
            ...
            if 'WGS84' in result.coordinate_system:
                click.echo(f"- Global reference point: ({ref_x:.6f}, {ref_y:.6f}, {ref_z:.2f})")
            else:
                click.echo(f"- Global reference point: ({ref_x:.2f}, {ref_y:.2f}, {ref_z:.2f} ft)")
            ...
            if 'coordinate_ranges' in result.details and result.details['coordinate_ranges']:
                ...
                for axis in ['x', 'y', 'z']:
                    if axis in ranges and axis in units:
                        ...
                        unit = units[axis]
                        if unit == 'degrees':
                            click.echo(f"- {axis.upper()} range: {min_val:.6f} to {max_val:.6f} {unit}")
                        else:
                            click.echo(f"- {axis.upper()} range: {min_val:.1f} to {max_val:.1f} {unit}")
